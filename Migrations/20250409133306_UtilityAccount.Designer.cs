// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using RentalMgtSystem.Models;

#nullable disable

namespace RentalMgtSystem.Migrations
{
    [DbContext(typeof(AppDBContext))]
    [Migration("20250409133306_UtilityAccount")]
    partial class UtilityAccount
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "8.0.10")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("RentalMgtSystem.Models.TenantContract", b =>
                {
                    b.Property<int>("ContractID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ContractID"));

                    b.Property<string>("Details")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("DownpaymentDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("ElectricityBased")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("EndDate")
                        .HasColumnType("datetime2");

                    b.Property<double?>("RentalAmount")
                        .HasColumnType("float");

                    b.Property<DateTime?>("StartDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("TenantName")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UnitName")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("ContractID");

                    b.ToTable("TenantContract");
                });

            modelBuilder.Entity("RentalMgtSystem.Models.Unit", b =>
                {
                    b.Property<int>("UnitID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("UnitID"));

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UnitName")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("UnitID");

                    b.ToTable("Unit");
                });

            modelBuilder.Entity("RentalMgtSystem.Models.Utility", b =>
                {
                    b.Property<int>("UtilityID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("UtilityID"));

                    b.Property<double?>("BillingAmount")
                        .HasColumnType("float");

                    b.Property<DateTime?>("BillingDate")
                        .HasColumnType("datetime2");

                    b.Property<double?>("MainMeterReading")
                        .HasColumnType("float");

                    b.Property<DateTime?>("PaymentDate")
                        .HasColumnType("datetime2");

                    b.Property<double?>("SubmeterReading")
                        .HasColumnType("float");

                    b.Property<string>("UtilityAccountNo")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UtilityType")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("UtilityID");

                    b.ToTable("Utility");
                });

            modelBuilder.Entity("RentalMgtSystem.Models.UtilityAccount", b =>
                {
                    b.Property<int>("UtilityAccountID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("UtilityAccountID"));

                    b.Property<int?>("UnitID")
                        .HasColumnType("int");

                    b.Property<string>("UtilityAccountNo")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("UtilityType")
                        .HasColumnType("int");

                    b.HasKey("UtilityAccountID");

                    b.HasIndex("UnitID");

                    b.ToTable("UtilityAccount");
                });

            modelBuilder.Entity("RentalMgtSystem.Models.UtilityAccount", b =>
                {
                    b.HasOne("RentalMgtSystem.Models.Unit", "Unit")
                        .WithMany()
                        .HasForeignKey("UnitID");

                    b.Navigation("Unit");
                });
#pragma warning restore 612, 618
        }
    }
}
